  ...
  <div class="app-container">
    ...
    <el-form v-show="showSearch" :inline="true" ref="form" :model="params" label-width="68px">
      ...
      <el-form-item label="标签名">
        <el-select style="width: 130px" size="small" v-model="params.tagId" filterable clearable reserve-keyword @change='handleFind' placeholder="请选择标签">
          <el-option v-for="item in tags" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="分类名">
        <el-select style="width: 130px" size="small" v-model="params.categoryId" filterable clearable reserve-keyword @change='handleFind' placeholder="请选择分类">
          <el-option v-for="item in category" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="发布状态">
        <el-select style="width: 130px" size="small" v-model="params.isPublish" filterable clearable reserve-keyword placeholder="是否发布" @change='handleFind'>
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button v-if="canReptile" size="small" class="filter-item" @click="dialogTableVisible = true" type="primary" icon="el-icon-cpu">文章抓取
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button size="small" v-if="canUpdate" :disabled="!multipleSelection.length" class="filter-item" @click="seo" type="info" icon="el-icon-position">批量SEO
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button size="small" :disabled="!multipleSelection.length" v-if="canDelBatch" class="filter-item" @click="handleDelete" type="danger" icon="el-icon-delete">批量删除
        </el-button>
      </el-col>
    ...
    <div style="margin-top: 5px">
      <el-table border :data="tableData" style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column align="center" width="116" label="类型">
          <template slot-scope="scope">
            <el-tag :type="scope.row.isOriginal === 0?'warning':'success'" :key="index" v-if="scope.row.isOriginal===index" v-for="(item, index) in isOriginalList">{{ item }} </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" width="220" label="标签">
          <template slot-scope="scope">
            <el-tag style="margin-left: 3px" align="center" type="primary" v-for="(item) in strSplit(scope.row.tagNames)" :key="item">{{item}}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="isStick" align="center" width="120" label="置顶">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.isStick" :active-value="1" :inactive-value="0" @change="handleTop(scope)" active-color="#13ce66">
            </el-switch>
          </template>
        </el-table-column>
      ...
      <div class="pagination-container" style="float: right;margin-bottom: 1.25rem;margin-top: 1.25rem;">
        <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="params.pageNo" :page-size="params.pageSize" :page-sizes="[10, 20, 30]" layout="total, sizes,prev, pager, next,jumper" :total="total">
        </el-pagination>
      </div>
