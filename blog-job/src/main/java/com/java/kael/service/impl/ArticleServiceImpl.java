...
import com.java.kael.common.*;
import com.java.kael.dto.ArticleDTO;
import com.java.kael.entity.*;
import com.java.kael.enums.PublishEnum;
import com.java.kael.enums.SearchModelEnum;
import com.java.kael.enums.YesOrNoEnum;
import com.java.kael.exception.BusinessException;
import com.java.kael.mapper.ArticleMapper;
import com.java.kael.mapper.CategoryMapper;
import com.java.kael.mapper.CommentMapper;
import com.java.kael.mapper.TagsMapper;
import com.java.kael.service.ArticleService;
import com.java.kael.service.RedisService;
import com.java.kael.service.SystemConfigService;
import com.java.kael.util.BeanCopyUtils;
import com.java.kael.util.ElasticsearchUtil;
import com.java.kael.util.IpUtils;
import com.java.kael.util.PageUtils;
import com.java.kael.vo.*;
import com.java.kael.strategy.context.SearchStrategyContext;
...
public class ArticleServiceImpl extends ServiceImpl<ArticleMapper, BlogArticle> implements ArticleService {
    ...
    public ResponseResult updateArticle(ArticleDTO article) {
        ...
        if (ObjectUtil.isNull(blogArticle)) {
            throw new BusinessException(ResultCode.ARTICLE_NOT_EXIST.getDesc());
        }
    ...
    public ResponseResult reptile(String url) {
        try {
            ...
            String strResult = restTemplate.getForObject(Constants.IMG_URL_API, String.class);
            JSONObject jsonObject = JSON.parseObject(strResult);
            Object imgUrl = jsonObject.get("imgurl");

            BlogArticle entity = BlogArticle.builder().userId(7L).contentMd(markdown)
                    .categoryId(Constants.OTHER_CATEGORY_ID).isOriginal(YesOrNoEnum.NO.getCode()).originalUrl(url)
    ...
    public ResponseResult randomImg() {
        //文章封面图片 由https://api.btstu.cn/该网站随机获取
        String result = restTemplate.getForObject(Constants.IMG_URL_API, String.class);
    ...
    public ResponseResult webArticleInfo(Integer id) {
        ...
        //最新文章
        List<LatestArticleVO> blogArticles = baseMapper.getNewArticles(id, PublishEnum.PUBLISH.code);
        blogArticle.setNewestArticleList(blogArticles);

        // 查询上一篇下一篇文章
        LatestArticleVO lastArticle = baseMapper.getNextOrLastArticle(id,0, PublishEnum.PUBLISH.code);
        blogArticle.setLastArticle(lastArticle);
        LatestArticleVO nextArticle = baseMapper.getNextOrLastArticle(id,1, PublishEnum.PUBLISH.code);
        ...
        //增加文章阅读量
        redisService.incrArticle(id.longValue(), RedisConstants.ARTICLE_READING);
    ...
    public ResponseResult condition(Long categoryId, Long tagId, Integer pageSize) {
        ...
        if (categoryId != null){
            ...
            redisService.incrArticle(tagId, RedisConstants.TAG_CLICK_VOLUME);
        }
        result.put(FieldConstants.NAME,name);
        result.put(Constants.RECORDS,blogArticlePage.getRecords());
    ...
    public ResponseResult articleLike(Integer articleId) {
        ...
        if (redisService.sIsMember(articleLikeKey, articleId)) {
            ...
            // 文章点赞量-1
            redisService.hDecr(RedisConstants.ARTICLE_LIKE_COUNT, articleId.toString(), 1L);
        } else {
            // 未点赞则增加文章id
            redisService.sAdd(articleLikeKey, articleId);
            // 文章点赞量+1
            redisService.hIncr(RedisConstants.ARTICLE_LIKE_COUNT, articleId.toString(), 1L);
        }
        return ResponseResult.success();
    }
    ...
    public ResponseResult checkSecret(String code) {
        ...
        if (ObjectUtil.isNull(redisCode)) {
            throw new BusinessException(ResultCode.ERROR_EXCEPTION_MOBILE_CODE.getDesc());
        }
        ...
        redisService.setCacheList(RedisConstants.CHECK_CODE_IP,cacheList);
        //通过删除验证码
